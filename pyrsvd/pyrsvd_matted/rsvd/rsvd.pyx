...
class RSVD(object):
    ...
    def __call__(self,movie_id,user_id,user_map=None):
        ...
        # min_rating=self.min_rating
        # max_rating=self.max_rating
        r=self.u[movie_id-1,0] + self.v[user_id,0] + np.dot(self.u[movie_id-1,1:],self.v[user_id,1:])
        #if r>max_rating:
        #    r=max_rating
        #if r<min_rating:
        #    r=min_rating
    ...
    def train(cls,factors,ratingsArray,dims,probeArray=None,\
                  ...
                  randomize=False, randomNoise=0.005, globalMean=0.0):
	...
	number of factors and m is the number of epochs to be performed. 
        ...
        globalMean : float
            The global mean of the data values (0.0)
        ...
        __trainModel(model,ratingsArray,probeArray,randomize=randomize,global_mean=globalMean)
        return model

def __trainModel(model,ratingsArray,probeArray,out=sys.stdout,randomize=False,global_mean=0.0):
...
cdef double predict(int uOffset,int vOffset, \
    ...
    pred += dataU[uOffset] + dataV[vOffset] # matted added this line
    cdef int k = 1 # matted changed this from 0 to 1
    for k from 1<=k<factors: # matted changed this from 0 to 1
...
cdef double train(Rating *ratings, \
                            double *dataU, double *dataV, \
                            int factors, int n, double reg,double lr,double GM=0.0):
    ...
    cdef int movie=0
    cdef Rating r
    cdef double uTemp=0.0,vTemp=0.0,err=0.0,sumSqErr=0.0, temp1=0.0, temp2=0.0
    for i from 0<=i<n:
        r=ratings[i]
        user=r.userID
        movie=r.movieID-1
        ...
        temp1 = predict(uOffset,vOffset, dataU, dataV, factors)
        sumSqErr+=err*err;

        uTemp = dataU[uOffset]
        vTemp = dataV[vOffset]

        dataU[uOffset]+=lr*(err-reg*(uTemp+vTemp-GM))
        dataV[vOffset]+=lr*(err-reg*(uTemp+vTemp-GM))
